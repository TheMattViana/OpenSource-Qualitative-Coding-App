...
export const AnalysisView: React.FC<Props> = ({ project, onClose, onExport }) => {
  ...
  const { chartData, filteredCodes, allSelections } = useMemo(() => {
    ...
    if (selectedFamilyId !== 'ALL') {
      const root = allCodes.find(c => c.id === selectedFamilyId);
      if (root) {
        displayCodes = allCodes.filter(c => c.id === root.id || c.parentId === root.id);
      }
    }
  ...
  }, [project.codes, project.selections, selectedFamilyId]);
        ...
        <div className="flex flex-1 overflow-hidden">
            ...
            <div className="w-64 bg-[var(--bg-main)] border-r border-[var(--border)] p-4 overflow-y-auto">
                <div className="flex items-center mb-4 text-[var(--text-muted)] font-bold text-sm uppercase tracking-wider">
                    <Filter size={14} className="mr-2" /> Code Families
                </div>
                <div className="space-y-1">
                    <button 
                        onClick={() => setSelectedFamilyId('ALL')}
                        className={`w-full text-left px-3 py-2 rounded-md text-sm font-medium transition-colors ${selectedFamilyId === 'ALL' ? 'bg-[var(--bg-panel)] shadow text-[var(--accent)]' : 'hover:bg-[var(--bg-panel)] text-[var(--text-muted)]'}`}
                    >
                        All Codes
                    </button>
                    {rootCodes.map(rootCode => (
                        <button
                            key={rootCode.id}
                            onClick={() => setSelectedFamilyId(rootCode.id)}
                            className={`w-full text-left px-3 py-2 rounded-md text-sm transition-colors flex items-center ${selectedFamilyId === rootCode.id ? 'bg-[var(--bg-panel)] shadow text-[var(--text-main)]' : 'hover:bg-[var(--bg-panel)] text-[var(--text-muted)]'}`}
                        >
